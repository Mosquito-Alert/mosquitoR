...
test_that("get_malert_data works with source as a file path", {
  ...
  jsonlite::write_json(list(list(version_UUID = "test-1", creation_year = curr_year)), json_file, auto_unbox = TRUE)
...
test_that("get_malert_data uses cache_path and reuses it", {
  ...
  dir.create(json_dir, recursive = TRUE)
  curr_year <- lubridate::year(lubridate::today())
  jsonlite::write_json(list(list(version_UUID = "cached-1", creation_year = curr_year)), 
  ...
  jsonlite::write_json(list(list(version_UUID = "reused-cache", creation_year = curr_year)), 
                      file.path(json_dir, paste0("all_reports", curr_year, ".json")), 
                      auto_unbox = TRUE)
  ...
  expect_message(result2 <- get_malert_data(source = "github", cache_path = cache_file), "Using cached file")
